...
interface SiteContent {
  ...
  cancellation_policy: string; form_active: string;
...
interface FormData {
  prenom: string; nom: string; email: string; telephone: string; motif: string; message: string;
}
...
const TICKER_SPEED: Record<string, string> = { slow: "50s", normal: "30s", fast: "15s" };
...
export default function Home() {
  ...
  const [formData, setFormData] = useState<FormData>({ prenom: "", nom: "", email: "", telephone: "", motif: "", message: "" });
  ...
  const tickerArr = siteContent.ticker_items
    ? siteContent.ticker_items.split("|").map((item) => item.trim()).filter(Boolean)
    : TICKER_DEFAULTS;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("contacts").insert([{
      ...
      subject: formData.motif, message: formData.message,
    ...
    <div>
      ...
      <nav className="nav">
        ...
        <ul className="navLinks">
          {(["about", "services", "approche", "tarifs", "contact"] as const).map((id) => (
            <li key={id}>
              <a href={`#${id}`} onClick={(e) => { e.preventDefault(); scrollTo(id); }}>
                {t(`nav.${id === "approche" ? "approach" : id === "tarifs" ? "pricing" : id}`)}
              </a>
            </li>
          ))}
        </ul>
        <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
          ...
          <a href="#contact" className="navCta" onClick={(e) => { e.preventDefault(); scrollTo("contact"); }}>
            {t("nav.cta")}
          </a>
          <button
      ...
      {menuOpen && (
        <div className="mobileMenu">
          ...
          <button className="mobileMenuCta" onClick={() => { setMenuOpen(false); setTimeout(() => scrollTo("contact"), 100); }}>
            {t("nav.cta")}
          </button>
        </div>
      )}
      ...
      <section className="hero">
        ...
        <div className="heroLeft">
          ...
          <h1 className="heroTitle"><em>{c("hero_subtitle")}</em></h1>
          <p className="heroQuote">« {c("hero_quote")} »</p>
          <p className="heroSub">{c("hero_tagline")}</p>
          <div className="heroActions">
            <a href="#contact" className="btnPrimary" onClick={(e) => { e.preventDefault(); scrollTo("contact"); }}>{t("hero.btn_contact")}</a>
            <a href="#services" className="btnGhost" onClick={(e) => { e.preventDefault(); scrollTo("services"); }}>{t("hero.btn_services")}</a>
          </div>
        </div>
        <div className="heroRight">
          <div className="heroImageWrap"><BotanicalSVG /></div>
      ...
      <div className="ticker">
        <div className="tickerTrack" style={{ animationDuration: tickerDuration }}>
      ...
      <section className="about" id="about">
        <div className="aboutVisual">
          <div style={{ position: "relative" }}>
            ...
            <div className="aboutBadge"><span>8+</span>{t("about.badge")}</div>
          </div>
        </div>
        <div className="aboutContent reveal">
          <p className="eyebrow">{t("about.eyebrow")}</p>
          <h2 className="sectionTitle">{t("about.title")} <em>{t("about.title_em")}</em></h2>
          <p className="text">{c("about_text_1")}</p>
          <p className="text">{c("about_text_2")}</p>
          <p className="text"><Trans i18nKey="about.paragraph3" components={{ strong: <strong /> }} /></p>
      ...
      <section className="services" id="services">
        <div className="servicesHeader reveal">
          ...
          <h2 className="sectionTitle">{t("services.title")} <em>{t("services.title_em")}</em></h2>
          <p className="text">{t("services.subtitle")}</p>
        </div>
        <div className="servicesGrid">
          {services.map((svc) => (
            <div key={svc.num} className="serviceCard reveal">
              <span className="serviceNum">{svc.num}</span>
              <div className="serviceIcon">{svc.icon}</div>
              <h3 className="serviceTitle">{t(svc.titleKey)}</h3>
              <p className="serviceDesc">{t(svc.descKey)}</p>
              <div className="serviceTags">
                {t(svc.tagsKey).split("|").map((tag) => (
                  <span key={tag} className="serviceTag">{tag}</span>
                ))}
              </div>
              <a href="#contact" className="serviceLink" onClick={(e) => { e.preventDefault(); scrollTo("contact"); }}>
                {t("services.link")}
              </a>
            </div>
          ))}
      ...
      <section className="process" id="approche">
        <div className="reveal">
          ...
          <h2 className="sectionTitle">{t("approach.title")} <em>{t("approach.title_em")}</em></h2>
        ...
        <div className="processVisual reveal">
          <div className="mandala"><div className="mandalaCenter">∞</div></div>
        </div>
      </section>
      ...
      {formations.length > 0 && (
        <section style={{ padding: "5rem 6rem", background: "var(--warm-white)" }} id="formations">
          <div className="reveal" style={{ textAlign: "center", maxWidth: 600, margin: "0 auto 3rem" }}>
            ...
            <h2 className="sectionTitle">{t("formations.title")} <em>{t("formations.title_em")}</em></h2>
          </div>
      ...
      <section className="pricing" id="tarifs">
        <div className="reveal">
          ...
          <h2 className="sectionTitle">{t("pricing.title")} <em>{t("pricing.title_em")}</em> {t("pricing.title_2")}</h2>
          <p className="text" style={{ maxWidth: "50ch", margin: "0 auto 3rem" }}>{t("pricing.subtitle")}</p>
        </div>
        <div className="pricingCards reveal">
          <div className="pricingCard">
            <p className="pricingLabel">{t("pricing.card1_label")}</p>
            <div className="pricingPrice">{t("pricing.card1_price")}</div>
            <p className="pricingDetail">{t("pricing.card1_detail")}</p>
            <p className="pricingNote">{t("pricing.card1_note")}</p>
            <button className="btnPricingOutline" onClick={() => scrollTo("contact")}>{t("pricing.card1_btn")}</button>
          </div>
          <div className="pricingCard pricingFeatured">
            <p className="pricingLabel">{t("pricing.card2_label")}</p>
            <div className="pricingPrice">{t("pricing.card2_price")}</div>
            <p className="pricingDetail">{t("pricing.card2_detail")}</p>
            <p className="pricingNote">{t("pricing.card2_note")}</p>
            <button className="btnPricingFilled" onClick={() => scrollTo("contact")}>{t("pricing.card2_btn")}</button>
          </div>
        </div>
      ...
      <section className="contact" id="contact">
        <div className="reveal">
          ...
          <h2 className="contactTitle">{t("contact.title")} <em>{t("contact.title_em")}</em></h2>
          <p className="text">{t("contact.subtitle")}</p>
          <div className="contactDetails">
            <div className="contactDetail">
              <div className="detailIcon"><PhoneIcon /></div>
              <a href={phoneHref}>{phone}</a>
            </div>
            <div className="contactDetail">
              <div className="detailIcon"><MailIcon /></div>
              <a href={`mailto:${email}`}>{email}</a>
            </div>
            <div className="contactDetail">
              <div className="detailIcon"><VideoIcon /></div>
      ...
      <footer className="footer">
        <div className="footerMain">
          <div>
            <div className="footerLogo">Nancy M <span>Therapy</span></div>
...
const langSwitch: CSSProperties = { display: "flex", alignItems: "center", gap: "0.3rem" };
const langBtn: CSSProperties = { background: "none", border: "none", cursor: "pointer", fontFamily: "Jost, sans-serif", fontSize: "0.72rem", letterSpacing: "0.12em", color: "var(--mist)", padding: "0.2rem 0.3rem", transition: "color 0.2s" };
const langBtnActive: CSSProperties = { color: "var(--charcoal)", fontWeight: 500 };

// Icons
function LifeIcon() {
  return <svg viewBox="0 0 48 48" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="24" cy="24" r="20"/><path d="M24 12 C24 12 16 18 16 26 C16 30.4 19.6 34 24 34 C28.4 34 32 30.4 32 26 C32 18 24 12 24 12Z"/><line x1="24" y1="34" x2="24" y2="38"/></svg>
}
function HeartIcon() {
  return <svg viewBox="0 0 48 48" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M24 40 C14 34 8 26 8 20 C8 14.5 12.5 10 18 10 C21 10 23.8 11.4 24 14 C24.2 11.4 27 10 30 10 C35.5 10 40 14.5 40 20 C40 26 34 34 24 40Z"/></svg>
}
function HypnoIcon() {
  return <svg viewBox="0 0 48 48" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="24" cy="18" r="10"/><path d="M14 28 C10 32 8 38 8 42 L40 42 C40 38 38 32 34 28"/><line x1="24" y1="8" x2="24" y2="5"/></svg>
}
function CoupleIcon() {
  return <svg viewBox="0 0 48 48" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="18" cy="20" r="10"/><circle cx="30" cy="28" r="10"/></svg>
}
function EyeIcon() {
  return <svg viewBox="0 0 48 48" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M8 24 C8 24 16 12 24 12 C32 12 40 24 40 24 C40 24 32 36 24 36 C16 36 8 24 8 24Z"/><circle cx="24" cy="24" r="6"/></svg>
}
function StarIcon() {
  return <svg viewBox="0 0 48 48" fill="none" stroke="currentColor" strokeWidth="1.5"><polygon points="24,6 28,17 40,17 30,24 34,35 24,28 14,35 18,24 8,17 20,17"/></svg>
}
function PhoneIcon() {
...
function MailIcon() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
}
function VideoIcon() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>
}
