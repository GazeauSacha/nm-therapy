...
export default function Messages() {
  ...
  async function createRdvFromContact() {
    ...
    alert(`Fonctionnalité : Créer un RDV pour ${selected.first_name} ${selected.last_name}\nVous pouvez aller dans Rendez-vous > Nouveau RDV.`)
  }

  // Send reply via Supabase Edge Function (or just log for now)
  async function sendReply(e) {
    ...
    setSending(false)
    setReply('')
    if (!error) alert(`✓ Réponse enregistrée. Pour envoyer l'email, configurez une Edge Function Supabase ou Resend.`)
  }
  ...
  return (
    <div>
      <Topbar title="Messages" subtitle={`${unreadCount} message${unreadCount !== 1 ? 's' : ''} non lu${unreadCount !== 1 ? 's' : ''}`} />
      <div style={s.layout}>
        {/* List */}
        <div style={s.list}>
          ...
          {loading ? <div style={s.empty}>Chargement…</div> : contacts.length === 0 ? (
            <div style={s.empty}>Aucun message reçu.</div>
          ) : contacts.map(c => (
            <div key={c.id} style={{ ...s.msgItem, ...(selected?.id === c.id ? s.msgSelected : {}), ...(c.read ? {} : s.msgUnread) }} onClick={() => selectContact(c)}>
          ...
          {!selected ? (
            ...
            <>
              <div style={s.detailHeader}>
                ...
                <button style={s.rdvBtn} onClick={createRdvFromContact}>+ Créer un RDV</button>
              </div>

              <div style={s.msgBubble}>
                {selected.subject && <div style={s.msgSubject}>Motif : {selected.subject}</div>}
                <p style={{ fontSize: '0.9rem', color: 'var(--charcoal)', lineHeight: 1.7 }}>{selected.message}</p>
              ...
              <form onSubmit={sendReply} style={s.replyForm}>
                ...
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                  ...
                  <a href={`mailto:${selected.email}?subject=Re: ${selected.subject || 'Votre demande NM Therapy'}&body=Bonjour ${selected.first_name},%0D%0A%0D%0A`} style={s.mailtoBtn}>Ouvrir dans Gmail →</a>
                </div>
                <p style={{ fontSize: '0.72rem', color: 'var(--mist)', marginTop: '0.5rem', fontStyle: 'italic' }}>
                  💡 Le lien "Ouvrir dans Gmail" lance votre client email avec la réponse pré-remplie.
                </p>
              </form>
            </>
          )}
...
const s = {
  ...
  detailHeader: { display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.5rem 2rem', background: 'var(--warm-white)', borderBottom: '1px solid rgba(139,158,126,0.1)' },
  msgBubble: { margin: '1.5rem 2rem', background: 'var(--warm-white)', borderRadius: 6, padding: '1.5rem', border: '1px solid rgba(139,158,126,0.15)' },
  ...
  replyForm: { margin: '0 2rem 2rem', background: 'var(--warm-white)', borderRadius: 6, padding: '1.5rem', border: '1px solid rgba(139,158,126,0.15)' },
