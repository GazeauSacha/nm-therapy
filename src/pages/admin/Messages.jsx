...
export default function Messages() {
  ...
  const [rdvForm, setRdvForm] = useState({ date: '', platform: 'Google Meet', subject: '', notes: '' });
  ...
  async function loadReplies(contactId) {
    const { data } = await supabase
      .from("replies")
      .select("*")
  ...
  async function sendReply(e) {
    ...
    try {
      ...
      // 3. Afficher immédiatement la réponse dans le fil
      if (newReply) {
  ...
  async function createRdv(e) {
    e.preventDefault();
    if (!selected) return;
    setSavingRdv(true);

    try {
      // 1. Trouver ou créer le client lié à ce contact
      let clientId = null;
      ...
      // 3. Envoyer un message automatique dans le chat
          ...
          {!selected ? (
            ...
            <>
              <div style={s.detailHeader}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: "1rem", fontWeight: 500, color: "var(--charcoal)" }}>
                ...
                <button style={s.rdvBtn} onClick={() => setShowRdv(true)}>
                  📅 Proposer un RDV
                </button>
              </div>
              ...
              <div style={s.thread}>
                ...
                {replies.map((r) => (
                  <div key={r.id} style={s.bubbleAdmin}>
                    <p style={{ fontSize: "0.9rem", color: "var(--warm-white)", lineHeight: 1.7, margin: 0, whiteSpace: 'pre-wrap' }}>
                      {r.message}
                    </p>
                    <div style={{ ...s.msgMeta, color: 'rgba(253,251,247,0.6)', textAlign: 'right' }}>
            ...
            <form onSubmit={createRdv}>
              ...
              <div style={s.formGroup}>
                ...
                <textarea
                  ...
                  rows={3}
                  value={rdvForm.notes}
                  onChange={e => setRdvForm(p => ({ ...p, notes: e.target.value }))}
                />
              </div>
              <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1.5rem' }}>
                <button type="submit" style={s.sendBtn} disabled={savingRdv}>
                  {savingRdv ? 'Enregistrement…' : 'Confirmer & envoyer'}
                </button>
                <button type="button" style={s.mailtoBtn} onClick={() => setShowRdv(false)}>
                  Annuler
                </button>
              </div>
            </form>
...
const s = {
  ...
  modal: {
    background: 'var(--warm-white)', borderRadius: 8, padding: '2.5rem',
    width: 480, maxWidth: '90vw', boxShadow: '0 24px 60px rgba(0,0,0,0.2)',
  },
  modalTitle: {
  ...
  formInput: {
    width: '100%', border: '1px solid rgba(139,158,126,0.25)', borderRadius: 3,
    padding: '0.7rem 0.9rem', fontFamily: 'Jost, sans-serif', fontSize: '0.85rem',
